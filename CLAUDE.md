...
**Idempotency**: `/banyan-c4` is safe to re-run. It tracks per-directory content hashes in `memory-bank/c4/c4-manifest.md` and only re-walks subtrees whose source has changed. Higher levels regenerate only when their inputs change.
