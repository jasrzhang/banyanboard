...
| 2026-06-01 | TASK-008 Basic User Administration | BUILD Phase 1/4 COMPLETE | Backend Foundation: users table migration (1747600008000_create-users.js), LoginSchema (Zod: min2/max30/letters+spaces/no-whitespace-only), UserRepository.findOrCreate (idempotent upsert — AC-SESSION-3), UserService.login, UserController (POST /api/users/login → 201 {id,firstName}, 400 on Zod fail matching existing error shape), usersRouter mounted at /api/users. 8 new integration tests + 112 existing = 120 total. Build+Lint PASS (1 pre-existing warning in errorHandler.ts). |
...
| 2026-05-30 | TASK-007 Card Workflow Automation | BUILD Phase 1/3 COMPLETE | Backend Foundation: automation_rules migration, AutomationRepository (CRUD + 2 action methods), AutomationService (cycle detection, fire-and-forget evaluate, layering-compliant — no direct pg in service), AutomationController (list/create/remove), automationsRouter (mounted at /api/boards with mergeParams:true, singleton export). CardController + LabelController wired with fire-and-forget evaluate hooks post-response. 18 new tests (15 automations.test.ts + 3 cards.test.ts trigger hooks) + 94 existing = 112 total. Build+Lint PASS. Code Review: APPROVED (layering fix: removed Pool from AutomationService; position fix: moveCardToColumn now appends at MAX+1000). |
